...
[https://learn-video.azurefd.net/vod/player?id=7b1f8d3d-ae64-4f52-a5b7-252ba78429af](https://learn-video.azurefd.net/vod/player?id=7b1f8d3d-ae64-4f52-a5b7-252ba78429af)
...
- [*Strong*](https://learn.microsoft.com/en-us/azure/cosmos-db/consistency-levels?context=/azure/cosmos-db/context/context#strong-consistency)
- [*Bounded staleness*](https://learn.microsoft.com/en-us/azure/cosmos-db/consistency-levels?context=/azure/cosmos-db/context/context#bounded-staleness-consistency)
- [*Session*](https://learn.microsoft.com/en-us/azure/cosmos-db/consistency-levels?context=/azure/cosmos-db/context/context#session-consistency)
- [*Consistent prefix*](https://learn.microsoft.com/en-us/azure/cosmos-db/consistency-levels?context=/azure/cosmos-db/context/context#consistent-prefix-consistency)
- [*Eventual*](https://learn.microsoft.com/en-us/azure/cosmos-db/consistency-levels?context=/azure/cosmos-db/context/context#eventual-consistency)

For more information on the default consistency level, see [configuring the default consistency level](https://learn.microsoft.com/en-us/azure/cosmos-db/how-to-manage-consistency#configure-the-default-consistency-level) or [override the default consistency level](https://learn.microsoft.com/en-us/azure/cosmos-db/how-to-manage-consistency#override-the-default-consistency-level).

Each level balances availability and performance. The following image shows the consistency levels as a spectrum.

![Diagram of consistency as a spectrum starting with strong and progressing to higher availability, throughput, and lower latency with eventual.](https://learn.microsoft.com/en-us/azure/cosmos-db/media/consistency-levels/five-consistency-levels.png)

## Consistency levels and Azure Cosmos DB APIs

Azure Cosmos DB supports wire protocol-compatible APIs for popular databases, including MongoDB, Apache Cassandra, Apache Gremlin, and Azure Table Storage. For the API for Gremlin or Table, Azure Cosmos DB uses the default consistency level configured on the account. To learn about consistency level mapping, see [API for Cassandra consistency mapping](https://learn.microsoft.com/en-us/azure/cosmos-db/cassandra/consistency-mapping) for Apache Cassandra and [API for MongoDB consistency mapping](https://learn.microsoft.com/en-us/azure/cosmos-db/mongodb/consistency-mapping) for MongoDB.
...
Configure the default consistency level on your Azure Cosmos DB account at any time. The default consistency level configured on your account applies to all Azure Cosmos DB databases and containers under that account. All reads and queries issued against a container or a database use the specified consistency level by default. When you change your account-level consistency, redeploy your applications and make any necessary code modifications to apply these changes. Learn more in [how to configure the default consistency level](https://learn.microsoft.com/en-us/azure/cosmos-db/how-to-manage-consistency#configure-the-default-consistency-level). You can also override the default consistency level for a specific request. Learn more in the [override the default consistency level](https://learn.microsoft.com/en-us/azure/cosmos-db/how-to-manage-consistency#override-the-default-consistency-level) article.
...
![Animation showing strong consistency level with musical notes that are always synced.](https://learn.microsoft.com/en-us/azure/cosmos-db/media/consistency-levels/strong-consistency.gif)
...
![Animation of bounded staleness consistency level using music notes that are eventually synced within a predefined delay of time or versions.](https://learn.microsoft.com/en-us/azure/cosmos-db/media/consistency-levels/bounded-staleness-consistency.gif)
...
![Animation of session consistency level using music notes that are synced within a single client session.](https://learn.microsoft.com/en-us/azure/cosmos-db/media/consistency-levels/session-consistency.gif)
...
![Animation of consistent prefix level using music notes that are synced eventually but as a transaction that isn't out of order.](https://learn.microsoft.com/en-us/azure/cosmos-db/media/consistency-levels/consistent-prefix.gif)
...
![Animation showing eventual consistency level with music notes that are eventually synced, but not within a specific bound.](https://learn.microsoft.com/en-us/azure/cosmos-db/media/consistency-levels/eventual-consistency.gif)
...
- [High-level TLAâº specifications for the five consistency levels offered by Azure Cosmos DB](https://github.com/tlaplus/azure-cosmos-tla)
...
- [Configure the default consistency level](https://learn.microsoft.com/en-us/azure/cosmos-db/how-to-manage-consistency#configure-the-default-consistency-level)
- [Override the default consistency level](https://learn.microsoft.com/en-us/azure/cosmos-db/how-to-manage-consistency#override-the-default-consistency-level)
