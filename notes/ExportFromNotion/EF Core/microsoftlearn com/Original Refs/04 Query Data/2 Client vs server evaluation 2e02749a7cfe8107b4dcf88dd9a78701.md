...
<aside>
💡 **TIP:** If you are using AsAsyncEnumerable and want to compose the query further on client side then you can use [System.Interactive.Async](https://www.nuget.org/packages/System.Interactive.Async/) library which defines operators for async enumerables. For more information, see [client side linq operators](https://learn.microsoft.com/en-us/ef/core/miscellaneous/async#client-side-async-linq-operators) .
...
Older EF Core versions supported client evaluation in any part of the query--not just the top-level projection. That's why queries similar to one posted under the [Unsupported client evaluation](https://learn.microsoft.com/en-us/ef/core/querying/client-eval#unsupported-client-evaluation) section worked correctly. Since this behavior could cause unnoticed performance issues, EF Core logged a client evaluation warning. For more information on viewing logging output, see [Logging](https://learn.microsoft.com/en-us/ef/core/logging-events-diagnostics/).
